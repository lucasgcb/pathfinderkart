...
class AlignDelegate(QItemDelegate):
    ...
    def setEditorData(self, editor, index):
        text = index.data(PySide2.QtCore.Qt.EditRole) or index.data(
            PySide2.QtCore.Qt.DisplayRole)
        editor.setText(text)
...
class Gerente:
    def __init__(self, app, janela):
        ...
        detectorThread = Thread(name='detect', target=detect, args=(
            janela.interface, semafaro_detector, running_detector))
        scannerThread = Thread(name='scan', target=scan, args=(
            janela.interface, running_detector, interrupt_scanner, semafaro_scanner, semafaro_detector, self.table_model))
        detectorThread.start()
        scannerThread.start()
        janela.interface.box_nome.setText(
            "arquivo_" + datetime.now().strftime("%d-%m-%Y %H-%M-%S"))
        ...
        def salvar_manual():
            ...
            interface_error_msg(self.janela.interface,
                                "Salvo: " + fname + ".csv")
